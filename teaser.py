...
# SuGaR keeps the vanilla 3DGS code under gaussian_splatting/; add it to the
# module search path so the standard scene/renderer/utils imports all resolve.
sys.path.insert(
    0, os.path.join(os.path.dirname(os.path.abspath(__file__)), "gaussian_splatting")
)
...
def draw_points_on_image(points, colors, image, size=1):
    ...
    for point, color in zip(points, colors):
        ...
        r, g, b = color
        draw.ellipse(
            (x - size, y - size, x + size, y + size), fill=(int(r), int(g), int(b))
        )
...
def render_teaser(
    ...
    with torch.no_grad():
        ...
        # Override gaussians with a directly-specified .ply (e.g. SuGaR refined output)
