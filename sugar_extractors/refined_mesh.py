...
def extract_mesh_and_texture_from_refined_sugar(args):
    ...
    checkpoint = torch.load(
        refined_model_path, map_location=nerfmodel.device, weights_only=False
    )
    refined_sugar = SuGaR(
        nerfmodel=nerfmodel,
    ...
    if postprocess_mesh:
        ...
        with torch.no_grad():
            ...
            new_scales = refined_sugar._scales.reshape(len(face_mask), -1, 2)[
                face_mask
            ].view(-1, 2)
            new_quaternions = refined_sugar._quaternions.reshape(len(face_mask), -1, 2)[
                face_mask
            ].view(-1, 2)
            new_densities = refined_sugar.all_densities.reshape(len(face_mask), -1, 1)[
                face_mask
            ].view(-1, 1)
            new_sh_coordinates_dc = refined_sugar._sh_coordinates_dc.reshape(
                len(face_mask), -1, 1, 3
            )[face_mask].view(-1, 1, 3)
            new_sh_coordinates_rest = refined_sugar._sh_coordinates_rest.reshape(
