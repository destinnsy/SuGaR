...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Script to extract a mesh from a coarse SuGaR scene."
    )
    ...
    n_gaussians_per_surface_triangle_map = {
        args.n_vertices_in_mesh: args.gaussians_per_triangle,
    }
    ...
    CONSOLE.print(
        f"Surface mesh normal consistency factor: {surface_mesh_normal_consistency_factor}"
    )
    CONSOLE.print(
        f"Number of Gaussians per surface triangle: {n_gaussians_per_surface_triangle_map}"
    )
        ...
        if evaluate_vanilla:
            ...
            with torch.no_grad():
                for cam_idx in cam_indices:
                    # GT image
                    gt_img = (
                        nerfmodel_30k.get_test_gt_image(cam_idx)
                        .permute(2, 0, 1)
                        .unsqueeze(0)
                    )
            ...
            scene_results["3dgs_30k"]["lpips"] = (
                torch.tensor(gs_30k_lpipss).mean().item()
            )
                ...
                for decimation_target in decimation_targets:
                    for refinement_iterations in refinement_iterations_list:
                        n_gaussians_per_surface_triangle = (
                            n_gaussians_per_surface_triangle_map[decimation_target]
                        )

                        if use_uv_texture:
                            CONSOLE.print("Using UV texture for rendering.")
                            coarse_estimation_factor_str = str(
                                coarse_estimation_factor
                            ).replace(".", "")
                            surface_level_str = str(surface_level).replace(".", "")
                            textured_mesh_path = os.path.join(
                                "./output/refined_mesh/",
                                scene_name,
                            ...
                            CONSOLE.print(
                                f"Loading textured mesh: {textured_mesh_path}"
                            )

                            textured_mesh = load_objs_as_meshes(
                                [textured_mesh_path]
                            ).to(nerfmodel_30k.device)
                            CONSOLE.print(
                                f"Loaded textured mesh with {len(textured_mesh.verts_list()[0])} vertices and {len(textured_mesh.faces_list()[0])} faces."
                            )
                            ...
                            mesh_raster_settings = RasterizationSettings(
                                image_size=(
                                    nerfmodel_30k.image_height,
                                    nerfmodel_30k.image_width,
                                ),
                                blur_radius=0.0,
                            ...
                            ).replace("YY", str(coarse_normal_factor).replace(".", ""))
                            ...
                            if use_marching_cubes:
                                CONSOLE.print(
                                    "Using Marching Cubes for mesh extraction."
                                )
                                sugar_mesh_path = (
                                    "sugarmesh_"
                                ...
                                CONSOLE.print(
                                    "Using Poisson Center for mesh extraction."
                                )
                                sugar_mesh_path = (
                                    "sugarmesh_"
                                    + sugar_checkpoint_path.split("/")[-2].replace(
                            ...
                            mesh_save_dir = os.path.join(
                                "./output/coarse_mesh/", scene_name
                            )
                            sugar_mesh_path = os.path.join(
                                mesh_save_dir, sugar_mesh_path
                            )
                            ...
                            refined_sugar_path = (
                                "sugarfine_"
                                + mesh_name.replace("sugarmesh_", "")
                                + "_normalconsistencyXX_gaussperfaceYY/"
                            )
                            refined_sugar_path = os.path.join(
                                os.path.join("./output/refined/", scene_name),
                                refined_sugar_path,
                            )
                            refined_sugar_path = refined_sugar_path.replace(
                                "XX",
                                str(surface_mesh_normal_consistency_factor).replace(
                                    ".", ""
                                ),
                            ).replace(
                                "YY",
                                str(n_gaussians_per_surface_triangle).replace(".", ""),
                            )
                            refined_sugar_path = os.path.join(
                                refined_sugar_path, f"{refinement_iterations}.pt"
                            )
                            CONSOLE.print(
                                f"Loading SuGaR model config {refined_sugar_path}..."
                            )
                            checkpoint = torch.load(
                                refined_sugar_path,
                                map_location=nerfmodel_30k.device,
                                weights_only=False,
                            )
                            refined_sugar = SuGaR(
                                ...
                                colors=SH2RGB(
                                    checkpoint["state_dict"]["_sh_coordinates_dc"][
                                        :, 0, :
                                    ]
                                ),
                                initialize=False,
                        ...
                        with torch.no_grad():
                            ...
                            for cam_idx in cam_indices:
                                ...
                                if use_uv_texture:
                                    p3d_cameras = (
                                        nerfmodel_30k.test_cameras.p3d_cameras[cam_idx]
                                    )
                                    sugar_img = (
                                        renderer(textured_mesh, cameras=p3d_cameras)[
                                            0, ..., :3
                                        ]
                                        .clamp(min=0, max=1)
                                        .permute(2, 0, 1)
                                    ...
                                    sugar_img = (
                                        refined_sugar.render_image_gaussian_rasterizer(
                                            nerf_cameras=nerfmodel_30k.test_cameras,
                                            camera_indices=cam_idx,
                                            verbose=False,
                                            bg_color=None,
                                ...
                                sugar_lpipss.append(
                                    lpips(sugar_img, gt_img, net_type="vgg")
                                )

                        CONSOLE.print(
                            f"Evaluation of SuGaR finished, with surface level {surface_level} and decimation target {decimation_target} and refinement iterations {refinement_iterations}."
                        )
                        str_surface_level = str(surface_level).replace(".", "")
                        scene_results[
                            f"sugar_{str_surface_level}_{decimation_target}_{refinement_iterations}"
                        ] = {}
                        scene_results[
                            f"sugar_{str_surface_level}_{decimation_target}_{refinement_iterations}"
                        ]["ssim"] = (torch.tensor(sugar_ssims).mean().item())
                        scene_results[
                            f"sugar_{str_surface_level}_{decimation_target}_{refinement_iterations}"
                        ]["psnr"] = (torch.tensor(sugar_psnrs).mean().item())
                        scene_results[
                            f"sugar_{str_surface_level}_{decimation_target}_{refinement_iterations}"
                        ]["lpips"] = (torch.tensor(sugar_lpipss).mean().item())
